...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        configs.pretrained_filename = os.path.join(
            configs.model_path, "tools", "objdet_models", "resnet", "pretrained", "fpn_resnet_18_epoch_300.pth"
        )
...
def detect_objects(input_bev_maps, model, configs):
    ...
    conv_x = configs.lim_x[1] - configs.lim_x[0]
    conv_y = configs.lim_y[1] - configs.lim_y[0]
    ## step 1 : check whether there are any detections

        ## step 2 : loop over all detections
    for detect in detections:
        score, bev_x, bev_y, z, h, bev_w, bev_l, yaw = detect
        
            ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure
        x = bev_y / configs.bev_height * conv_x
        y = bev_x / configs.bev_width * conv_y - conv_y / 2.0
        w = bev_w / configs.bev_width * conv_y
        l = bev_l / configs.bev_height * conv_x
